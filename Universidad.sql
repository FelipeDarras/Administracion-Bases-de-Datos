...
select * from cohorte where idCohorte not in (select idCohorte from alumno);
select distinct idCohorte  from alumno;

select idCohorte, count(*) from alumno group by idCohorte;

DELETE FROM cohorte where idCohorte = 1243;
-- alumnocarreracarreraDELETE FROM cohorte where idCohorte = 1244;
...
select * from alumno where cedulaIdentidad = '41944781';

select distinct cedulaIdentidad,nombre,apellido,fechaNacimiento,fechaIngreso,idCohorte
from alumno
where cedulaIdentidad = '41944781';

select distinct cedulaIdentidad,nombre,apellido,fechaNacimiento,fechaIngreso,idCohorte from alumno;
CREATE TABLE alumno_sinduplicado (
...
INSERT INTO alumno_sinduplicado (cedulaIdentidad,nombre,apellido,fechaNacimiento,fechaIngreso,idCohorte)
select distinct cedulaIdentidad,nombre,apellido,fechaNacimiento,fechaIngreso,idCohorte from alumno;
...
select max(fechaIngreso) as max_fechaIngreso, min(fechaIngreso) as min_fechaIngreso from alumno;
select distinct cedulaIdentidad,nombre,apellido,fechaNacimiento,fechaIngreso,idCohorte
from alumno where cedulaIdentidad = '41944781';

select cedulaIdentidad,nombre,apellido,fechaNacimiento,fechaIngreso,idCohorte
from alumno 
where cedulaIdentidad = '41944781'
group by cedulaIdentidad,nombre,apellido,fechaNacimiento,fechaIngreso,idCohorte;

select idCohorte
from alumno ;

select idCohorte, count(distinct cedulaIdentidad) as cant_alumnos
from alumno 
group by idCohorte;

select cedulaIdentidad,nombre,apellido,fechaNacimiento,fechaIngreso,idCohorte ,count(*) as cant, min(idAlumno) as idAlumnoMin
from alumno
